...
public class AddressMap<T> {
   ...
   public void remove(final SimpleString key, Object value) {
   ...
   public void visitMatching(SimpleString address,
                             AddressMapVisitor<T> collector) throws Exception {
      final String[] paths = getPaths(address);
      rootNode.visitMatching(paths, 0, collector, true);
   }

   public void visitNonWildcard(SimpleString wildcardAddress,
                             AddressMapVisitor<T> collector) throws Exception {
      final String[] paths = getPaths(wildcardAddress);
      rootNode.visitMatching(paths, 0, collector, false);
   }
